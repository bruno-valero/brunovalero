...
export default async function readPdfAddQuizRoute(req:Request) {
    try {
        const { docId, uid, autoBuy, quizFocus } = await req.body as {  docId:string, uid:string, autoBuy?:boolean, quizFocus:string };
