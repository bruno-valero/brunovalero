...
export default async function readPdfSendQuestionRoute(req:Request) {
    try {
        const { question, docId, uid, autoBuy } = await req.body as { question: string, docId:string, uid:string, autoBuy?:boolean };
